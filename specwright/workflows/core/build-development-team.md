...
- **Step 9.5 Fix**: Custom Skill Detection runs as Main Agent (was: tech-architect Sub-Agent delegation)
...
- ❌ No `specwright/skills/` folder (old flat structure)
...
Both Product and Platform projects use specwright/product/tech-stack.md.

<conditional_logic>
  IF specwright/product/tech-stack.md exists:
...
Both Product and Platform projects use specwright/product/.
...
<context_loading>
  ...
  **Required Documents:**
  ...
  1. READ: specwright/product/tech-stack.md (already read in Step 1)
     EXTRACT:
     - Framework versions
  ...
  2. READ: Architecture document (try multiple locations)
     TRY:
       a) specwright/product/architecture-decision.md (Product projects)
       b) specwright/product/architecture/platform-architecture.md (Platform projects)
  ...
  3. READ: specwright/product/architecture-structure.md (if exists)
     EXTRACT:
  ...
  **Frontend-Specific Documents:**
  ...
  4. READ: specwright/product/design-system.md (if exists)
     EXTRACT:
  ...
  5. READ: specwright/product/ux-patterns.md (if exists)
     EXTRACT:
     - Navigation patterns
     - User flow patterns
...
<skill_creation>
  **1. Quality Gates Skill (alwaysApply: true)**
  ...
  LOAD template: specwright/templates/skills/quality-gates/SKILL.md
  (Fallback: ~/.specwright/templates/skills/quality-gates/SKILL.md)
  ...
  **2. PO Requirements Skill (for story creation)**
  ...
  LOAD template: specwright/templates/skills/po-requirements/SKILL.md
  (Fallback: ~/.specwright/templates/skills/po-requirements/SKILL.md)
  ...
  **3. Architect Refinement Skill (for story creation)**
  ...
  LOAD template: specwright/templates/skills/architect-refinement/SKILL.md
  (Fallback: ~/.specwright/templates/skills/architect-refinement/SKILL.md)
...
<conditional_logic>
  IF frontend framework detected (Angular/React/Vue):
    ...
    LOAD templates from:
    - specwright/templates/skills/frontend/[FRAMEWORK_NAME]/SKILL.md
    - specwright/templates/skills/frontend/[FRAMEWORK_NAME]/components.md
    - specwright/templates/skills/frontend/[FRAMEWORK_NAME]/state-management.md
    - specwright/templates/skills/frontend/[FRAMEWORK_NAME]/api-integration.md
    - specwright/templates/skills/frontend/[FRAMEWORK_NAME]/forms-validation.md
    - specwright/templates/skills/frontend/[FRAMEWORK_NAME]/dos-and-donts.md

    (Fallback to ~/.specwright/templates/... if project templates not found)
...
<conditional_logic>
  IF backend framework detected (Rails/NestJS/Spring):
    ...
    LOAD templates from:
    - specwright/templates/skills/backend/[FRAMEWORK_NAME]/SKILL.md
    - specwright/templates/skills/backend/[FRAMEWORK_NAME]/services.md
    - specwright/templates/skills/backend/[FRAMEWORK_NAME]/models.md
    - specwright/templates/skills/backend/[FRAMEWORK_NAME]/api-design.md
    - specwright/templates/skills/backend/[FRAMEWORK_NAME]/testing.md
    - specwright/templates/skills/backend/[FRAMEWORK_NAME]/dos-and-donts.md
...
<conditional_logic>
  IF DevOps tools detected (Docker, GitHub Actions, etc.):
    ...
    - specwright/templates/skills/devops/docker-github/docker.md
    - specwright/templates/skills/devops/docker-github/ci-cd.md
    - specwright/templates/skills/devops/docker-github/dos-and-donts.md
...
<step number="9.5" name="detect_custom_skills">

### Step 9.5: Detect and Generate Custom Skills (Optional)

Main agent analyzes tech-stack.md and existing code for specialized technologies that need custom skills beyond the standard framework skills already created.

<custom_skill_detection>
  READ: specwright/product/tech-stack.md (already in context from earlier steps)

  SCAN tech stack for specialized technologies not covered by standard skills:

  **Detection Checklist:**

  | Category | Look For | Trigger Libraries/Frameworks |
  |----------|----------|------------------------------|
  | Blockchain/Crypto | Wallet mgmt, smart contracts, DEX | ethers.js, web3.js, @solana/web3.js, wagmi, viem, CCXT |
  | Desktop/Electron | IPC, native modules | Electron, node-keytar, better-sqlite3, ffi-napi |
  | ML/AI | Model training, inference, embeddings | TensorFlow, PyTorch, OpenAI API, Anthropic SDK |
  | IoT | Device communication, sensors | MQTT, CoAP, sensor libraries |
  | Game Development | Game logic, physics | Unity, Unreal Engine, Godot |
  | Payment | Payment processing | Stripe, PayPal, Adyen |
  | Communication | SMS, Email APIs | Twilio, SendGrid |
  | Maps/Location | Geospatial features | Google Maps, Mapbox |
  | Real-time | WebSocket, live sync | Socket.io, WebRTC |
  | Mobile | Native modules, platform APIs | React Native, Flutter |

  IF specialized technology detected:

    FOR EACH detected technology:
      1. Research best practices (use WebSearch for [technology] best practices [CURRENT_YEAR])
      2. Create custom skill using templates:
         - LOAD template: specwright/templates/skills/custom-skill-template.md → .claude/skills/custom-[technology]/SKILL.md
         - LOAD template: specwright/templates/skills/custom-skill-module-template.md → for each aspect module
         - LOAD template: specwright/templates/skills/custom-skill-dos-and-donts-template.md → dos-and-donts.md
      3. Fill templates with:
         - YAML frontmatter with appropriate globs
         - Quick Reference (key patterns, when to use)
         - Detailed pattern modules as sub-documents
         - Code examples, security considerations, testing patterns
      4. Skill structure:
         .claude/skills/custom-[technology]/
         ├── SKILL.md
         ├── [aspect-1].md
         ├── [aspect-2].md
         └── dos-and-donts.md
  ...
  NOTE: Only create custom skills for specialized technologies.
  Standard frameworks (React, Angular, NestJS, Spring, etc.) are already covered by standard skills.
</custom_skill_detection>
...
<file_creation>
  CREATE directory: specwright/team/ (if not exists)

  LOAD template: specwright/templates/docs/dod-template.md
  (Fallback: ~/.specwright/templates/docs/dod-template.md)
  ...
  WRITE to: specwright/team/dod.md

  LOAD template: specwright/templates/docs/dor-template.md
  WRITE to: specwright/team/dor.md
</file_creation>
...
<output_summary>
  ...
  **Context Source:** specwright/product/
  ...
  **Optional Skills:**
  ...
  - Definition of Done: specwright/team/dod.md
  - Definition of Ready: specwright/team/dor.md
  ...
  [IF specialized technologies detected in Step 9.5:]
...
1. `specwright/templates/skills/...` (project)
2. `~/.specwright/templates/skills/...` (global)
